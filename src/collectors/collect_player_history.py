...
def load_master_ids():
    """Läs in masterfilen och returnera en set med giltiga player_ids (endast numeriska)."""
            ...
            for ev in match.get("events", []):
                for role in ["player", "player_in", "player_out", "assist_player"]:
                    ...
                    if p and "id" in p:
                        ...
                        # Endast spelare från masterlistan
...
def main():
    ...
    history = collect_player_history(args.league_id, args.season, africa_ids)
    ...
    print(f"[collect_player_history] Players included: {len(history)}")
